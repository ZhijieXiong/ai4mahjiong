...
def response():
    ...
    if "data" not in full_input:
        ...
        if "players_melds" not in full_input["data"]:
            full_input["data"]["players_melds"] = tuple([[] for _ in range(4)])
        if "players_played_card_actions" not in full_input["data"]:
            full_input["data"]["players_played_card_actions"] = tuple([[] for _ in range(4)])
        if "players_played_card_ids" not in full_input["data"]:
            full_input["data"]["players_played_card_ids"] = tuple([[] for _ in range(4)])
        if "num_draw" not in full_input["data"]:
            full_input["data"]["num_draw"] = [13 for _ in range(4)]
